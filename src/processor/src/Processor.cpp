...
void Processor::writeResult(){
    ofstream resultsStream("processor_results.csv");
    if (!resultsStream.is_open()) {
        std::cerr << "[ERROR] Failed to open results stream" << std::endl;
        raise(SIGTERM);
    }

    resultsStream << OneObjectRow::printHeader() << std::endl;

    for(unsigned int i = 0; i != m_result->size();i++) {
        const OneObjectRow& oor = m_result->getRow(i);
        resultsStream << oor.print() << std::endl;
    }
}